...
export function usePagination({ currentPage, totalPages, paginationItemsToDisplay }: Props) {
	const showLeftEllipsis = $derived(currentPage - 1 > paginationItemsToDisplay / 2);
	const showRightEllipsis = $derived(totalPages - currentPage + 1 > paginationItemsToDisplay / 2);
	...
	const pages = $derived(calculatePaginationRange());
