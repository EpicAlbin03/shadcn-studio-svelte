<script lang="ts">
	...
	let googleSwitch = $state(false);
	let twitterSwitch = $state(false);
	let linkedinSwitch = $state(false);
...
<DropdownMenu.Root>
	...
	>
		<DropdownMenu.Label>Apps notification</DropdownMenu.Label>
		<DropdownMenu.Group>
			<DropdownMenu.Item class="justify-between" onSelect={(event) => event.preventDefault()}>
