...
<div class="w-full max-w-md">
	<Tabs value="explore" orientation="vertical">
		<TabsList class="bg-background">
			{#each tabs as tab (tab.value)}
				<TabsTrigger
					value={tab.value}
					class="w-full data-active:bg-primary data-active:text-primary-foreground data-active:hover:text-primary-foreground dark:data-active:border-transparent dark:data-active:bg-primary dark:data-active:text-primary-foreground dark:data-active:hover:text-primary-foreground"
				>
					{tab.name}
				</TabsTrigger>
			{/each}
