...
<div class="w-full max-w-md">
	<Tabs value="explore" orientation="vertical">
		<TabsList class="rounded-none bg-background p-0">
			{#each tabs as tab (tab.value)}
				<TabsTrigger
					value={tab.value}
					class="w-full justify-start rounded-none border-0 border-l-2 border-transparent bg-background data-active:border-primary data-active:shadow-none dark:data-active:border-primary"
				>
					{tab.name}
				</TabsTrigger>
			{/each}
