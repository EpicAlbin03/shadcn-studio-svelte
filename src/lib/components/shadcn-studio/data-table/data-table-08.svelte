...
import {
  ...
  useSensors,
  type DragEndEvent
} from '$libdnd-kit/core'
import { restrictToHorizontalAxis } from '$libdnd-kit/modifiers'
import { arrayMove, horizontalListSortingStrategy, SortableContext, useSortable } from '$libdnd-kit/sortable'
import { CSS } from '$libdnd-kit/utilities'
import type { Cell, ColumnDef, Header, SortingState } from '$libtanstack/react-table'
...
const columns: ColumnDef<Employee>[] = [
  {
    ...
    cell: ({ row }) => <div class='font-medium'>{row.getValue('firstName')}</div>,
    sortUndefined: 'last',
    sortDescFirst: false
  },
  {
...
const DraggableColumnDataTableDemo = () => {
  ...
  return (
    <div class='w-full'>
          ...
          <Table>
            <TableHeader>
              {table.getHeaderGroups().map(headerGroup => (
                <TableRow key={headerGroup.id} class='bg-muted/50 [&>th]:border-t-0'>
      ...
      <p class='text-muted-foreground mt-4 text-center text-sm'>Data table with draggable columns</p>
    </div>
  )
}
      ...
      <div class='flex items-center justify-start gap-0.5'>
        ...
        >
          {{
            ...
            <ChevronUpIcon class='shrink-0 opacity-0 group-hover:opacity-60' size={16} aria-hidden='true' />
          )}
        </Button>
      </div>
    </TableHead>
...
const DragAlongCell = ({ cell }: { cell: Cell<Employee, unknown> }) => {
  ...
  return (
    <TableCell ref={setNodeRef} class='truncate' style={style}>
