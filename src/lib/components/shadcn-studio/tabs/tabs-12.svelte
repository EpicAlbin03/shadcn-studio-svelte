...
<div class="w-full max-w-md">
	<Tabs value="explore" class="gap-4">
		<TabsList class="rounded-none border-b bg-background p-0">
			{#each tabs as tab (tab.value)}
				<TabsTrigger
					value={tab.value}
					class="h-full rounded-none border-b-2 border-transparent bg-background data-[state=active]:border-primary data-[state=active]:shadow-none dark:data-[state=active]:border-primary"
				>
					{tab.name}
				</TabsTrigger>
			{/each}
