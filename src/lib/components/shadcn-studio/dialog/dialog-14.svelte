...
<Dialog.Root>
	<form>
		<Dialog.Trigger>
			{#snippet child({ props })}
				<Button {...props} variant="outline">Sign In</Button>
			{/snippet}
		</Dialog.Trigger>
		<Dialog.Content
		...
		>
			<Dialog.Header class="items-center">
				<div
					class="mb-4 flex size-12 items-center justify-center rounded-full bg-sky-600/10 sm:mx-0 dark:bg-sky-400/10"
				>
					<LogInIcon class="size-6 text-sky-600 dark:text-sky-400" />
				</div>
			...
			<Dialog.Footer class="space-y-2 pt-4 sm:flex-col">
				<Button
				...
				>
					<span class="text-xs text-muted-foreground">Or sign in with</span>
				</div>
