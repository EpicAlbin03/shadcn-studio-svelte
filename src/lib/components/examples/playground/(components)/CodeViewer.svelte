...
<Dialog.Root>
	...
	<Dialog.Content class="sm:max-w-[625px]">
		<Dialog.Header>
			...
			<Dialog.Description>
				You can use the following code to start integrating your current prompt and settings into
				your application.
			</Dialog.Description>
		</Dialog.Header>
				...
				<pre><code class="grid gap-1 text-sm text-muted-foreground [&_span]:h-4"
						...
						><span></span><span
							>openai.api_key = os.getenv(<span class="text-green-300"
								>&quot;OPENAI_API_KEY&quot;</span
							>)</span
						><span></span><span>response = openai.Completion.create(</span><span> model=<span
						...
						><span> max_tokens=<span class="text-amber-300">5</span>,</span><span> top_p=<span
								class="text-amber-300">1</span
							>,</span
						><span> frequency_penalty=<span class="text-amber-300">0</span>,</span
						><span> presence_penalty=<span class="text-green-300">0</span>,</span><span>)</span
						></code
					></pre>
			</div>
			<div>
